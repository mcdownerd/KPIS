...
import { Loader2, Star, Cloud } from "lucide-react";
import { upsertDigitalCommMetricsForStore } from "@/lib/api/service";
import { toast } from "sonner";

export function GoogleRatingsDashboard() {
    const { googleData: initialData, loading, error, refetch } = useDigitalCommData();
    ...
    const handleAutoSave = async (location: string, monthKey: string, value: string) => {
        ...
        try {
            ...
            console.error(err);
            toast.error(`Erro ao salvar Google Rating para ${location}`);
        } finally {
            setSavingCell(null);
        }
    };
        ...
        <div className="space-y-4">
            <div className="flex items-center justify-between px-1">
                <div className="flex items-center gap-2">
                    ...
                    <h3 className="text-lg font-semibold text-foreground">Evolução Mensal Google</h3>
                </div>
