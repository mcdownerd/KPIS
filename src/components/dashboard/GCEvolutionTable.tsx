...
export function GCEvolutionTable() {
    ...
    useEffect(() => {
        async function loadData() {
            try {
                ...
                console.error("Error loading GC evolution data:", error);
            } finally {
                setLoading(false);
            }
        }
    ...
    const getMonthValue = (data: SalesData[], month: string, storeId: string, field: 'totais' | 'delivery'): number => {
    ...
    const renderDataRow = (label: string, storeId: string, year: number, field: 'totais' | 'delivery') => {
        ...
        return (
            <TableRow className="border-b border-border/50 hover:bg-muted/10 h-9">
                ...
                {shortMonths.map((month) => {
                    const value = getMonthValue(data, month, storeId, field);
                    return (
    ...
    return (
        <div className="space-y-4 mt-8">
            <div className="flex items-center gap-2 px-1">
                ...
                <h3 className="text-lg font-semibold text-foreground uppercase tracking-wider">Evolução Relativa das GC's</h3>
            </div>
            ...
            <div className="overflow-x-auto rounded-md border border-border/50 bg-card/30">
                <Table>
                    <TableBody>
                        ...
                        {renderDataRow("TOTAIS vs 2024", storeMapping["Amadora (20)"], 2024, 'totais')}
                        {renderDeliveryRow()}
                        {renderDataRow("20 vs 2019", storeMapping["Amadora (20)"], 2019, 'delivery')}
                        {renderDataRow("TOTAIS vs 2024", storeMapping["Queluz (32)"], 2024, 'totais')}
                        {renderDeliveryRow()}
                    </TableBody>
                </Table>
            </div>
